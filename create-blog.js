...
function getAuthors() {
  ...
  try {
    ...
    console.error(
      `${COLORS.red}Error reading authors: ${error.message}${COLORS.reset}`
    );
  }
...
function getCategories() {
  ...
  try {
    ...
    console.error(
      `${COLORS.red}Error reading categories: ${error.message}${COLORS.reset}`
    );
  }
...
async function createBlogFiles(blogInfo) {
  ...
  try {
    ...
    if (blogInfo.coverSourcePath && blogInfo.coverTargetPath) {
      coverCopied = await copyImage(
        blogInfo.coverSourcePath,
        blogInfo.coverTargetPath
      );
    }
...
async function main() {
  ...
  if (authors.length === 0) {
    console.log(
      `${COLORS.yellow}⚠ No authors found in the system${COLORS.reset}`
    );
  } else {
    console.log(
      `${COLORS.pink}✓${COLORS.reset} Found ${authors.length} authors`
    );
  }

  if (categories.length === 0) {
    console.log(
      `${COLORS.yellow}⚠ No categories found in the system${COLORS.reset}`
    );
  } else {
    console.log(
      `${COLORS.pink}✓${COLORS.reset} Found ${categories.length} categories`
    );
  }
  ...
  try {
    ...
    console.log(
      `${COLORS.pink}Description:${COLORS.reset} ${blogInfo.description}`
    );
    ...
    if (result.success) {
      ...
      console.log(
        `  • Blog post: ${COLORS.pink}${result.blogPath}${COLORS.reset}`
      );
      console.log(
        `  • Images: ${COLORS.pink}${result.imagePath}${COLORS.reset}`
      );
